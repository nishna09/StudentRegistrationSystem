$(function () {
    ...
    $("button#updateBtn").click(function(){
        ...
        if ($("select#subject1").val() != -1 && $("select#grade1").val() != -1) {
            result.push({ Subject: { SubjectId: parseInt($("select#subject1").val()), SubjectName: null }, Grade: $("select#grade1").val() });
        }
        if ($("select#subject2").val() != -1 && $("select#grade2").val() != -1) {
            result.push({ Subject: { SubjectId: parseInt($("select#subject2").val()), SubjectName: null }, Grade: $("select#grade2").val() });
        }
        if ($("select#subject3").val() != -1 && $("select#grade3").val() != -1) {
            result.push({ Subject: { SubjectId: parseInt($("select#subject3").val()), SubjectName: null }, Grade: $("select#grade3").val() });
        }
        if (result.length == 0) {
